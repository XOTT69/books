...
function switchTab(tab){
    ...
    const idx={library:0,discover:1,profile:2}[tab];
...
function updateGoalWidget(){
    const g=parseInt(localStorage.getItem('readingGoal'))||12,y=new Date().getFullYear();
    const f=myLibrary.filter(b=>{if(b.status!=='finished')return false;try{return b.dateFinished&&b.dateFinished.startsWith(String(y));}catch(e){return false;}}).length;
...
auth.onAuthStateChanged(async user=>{
    ...
    if(user){
        currentUser=user;as.classList.add('hidden');as.classList.remove('flex');ap.classList.remove('hidden');
...
searchInput.addEventListener('input',e=>{
    ...
    searchItems.innerHTML='<div class="p-8 text-muted text-sm text-center animate-pulse">Шукаю...</div>';
...
function render(){
    ...
    document.getElementById('tab_reading').innerHTML=`📖 Читаю <span class="ml-1 text-[10px] opacity-70">${reading.length}</span>`;
    document.getElementById('tab_planned').innerHTML=`🕒 В планах <span class="ml-1 text-[10px] opacity-70">${planned.length}</span>`;
    ...
    if(currentLibraryTab==='finished'){
        ...
        let html='';Object.keys(gr).sort((a,b)=>{if(a==='—')return 1;if(b==='—')return-1;return b-a;}).forEach(y=>{html+=`<p class="font-black text-slate-200 text-sm mt-5 mb-2 tracking-widest">${y}</p><div class="${wc}">${gr[y].map(renderBookCard).join('')}</div>`;});c.innerHTML=html;
...
async function loadRealRecommendations(cat='auto',btnId='rec_auto'){
    ...
    const list=document.getElementById('recommendationsList');list.innerHTML='<div class="p-10 text-muted text-sm text-center animate-pulse">🔍 Шукаємо...</div>';
...
async function fetchMoreRecommendations(isFirst=false){
    ...
    if(!isFirst&&!document.getElementById('recLoadingMore')){const d=document.createElement('div');d.id='recLoadingMore';d.className='py-6 text-center text-muted animate-pulse text-sm';d.innerHTML='Ще...';lst.appendChild(d);}
