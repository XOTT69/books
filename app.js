...
async function updateStreakWidget() {
    ...
    if (streakDots) streakDots.innerHTML = days.map(d => `<div class="streak-cell ${rd.has(d) ? 'active' : 'inactive'} ${d === ts ? 'ring-2 ring-primary-300 ring-offset-1' : ''}" title="${d}"></div>`).join('');
}
...
function renderQuickResume() {
    ...
    qr.innerHTML = `<div class="card-elevated p-4 flex items-center gap-3 cursor-pointer active:scale-[0.98] transition-transform" onclick="readSavedEpub('${lastRead.id}')">
        <img src="${lastRead.image || PLACEHOLDER_IMG}" onerror="this.src='${PLACEHOLDER_IMG}'" class="w-10 h-14 rounded-lg object-cover shadow-sm">
        <div class="flex-1 min-w-0">
            <p class="text-[10px] font-bold text-primary-600 uppercase">Продовжити</p>
            <p class="font-bold text-sm truncate">${escapeHtml(lastRead.title)}</p>
            <div class="flex items-center gap-2 mt-1">
                <div class="flex-1 bg-slate-100 h-1.5 rounded-full overflow-hidden"><div class="bg-primary-500 h-full rounded-full" style="width:${pct}%"></div></div>
                <span class="text-[10px] font-bold text-muted">${pct}%</span>
            </div>
        </div>
        <div class="bg-primary-600 text-white size-9 rounded-full flex items-center justify-center text-lg shadow-md shadow-primary-200">▶</div>
    </div>`;
}
...
function renderBookCard(book) {
    ...
    if (viewMode === 'grid') {
        return `<div data-id="${book.id}" onclick="showBookDetailsById('${book.id}')" class="flex flex-col items-center cursor-pointer active:scale-[0.97] transition-transform">
            <div class="relative w-full aspect-[2/3]">
                <img loading="lazy" src="${img}" onerror="this.src='${PLACEHOLDER_IMG}'" class="w-full h-full rounded-2xl shadow-md object-cover">
                ${isFin && book.rating ? `<div class="absolute -bottom-1.5 -right-1.5 bg-white text-amber-400 text-[9px] font-black px-1.5 py-0.5 rounded-md shadow-sm">★${book.rating}</div>` : ''}
                ${isRead && pct > 0 ? `<div class="absolute bottom-0 left-0 right-0 h-1 bg-black/20 rounded-b-2xl overflow-hidden"><div class="h-full bg-primary-500" style="width:${pct}%"></div></div>` : ''}
            </div>
            <h3 class="font-semibold text-[11px] mt-2 w-full text-center truncate px-1">${title}</h3>
        </div>`;
    }
    return `<div data-id="${book.id}" onclick="showBookDetailsById('${book.id}')" class="card p-3.5 flex gap-3 items-start cursor-pointer">
        <img loading="lazy" src="${img}" onerror="this.src='${PLACEHOLDER_IMG}'" class="w-14 h-20 rounded-xl shadow-sm object-cover flex-shrink-0">
        <div class="flex-1 min-w-0">
            <div class="flex justify-between items-start gap-1">
                <div class="min-w-0">
                    <h3 class="font-bold text-sm leading-snug truncate">${title}</h3>
                    <p class="text-xs text-muted mt-0.5 truncate">${author}</p>
                </div>
                <button onclick="deleteBook('${book.id}',event)" class="text-slate-200 hover:text-red-400 text-xs p-1 shrink-0">✕</button>
            </div>
            ${isPlan ? `<button onclick="changeStatus('${book.id}','reading',event)" class="w-full mt-3 py-2 bg-primary-50 text-primary-700 font-bold text-xs rounded-lg active:scale-95">🚀 Почати читати</button>` :
        `<div class="mt-2.5 flex items-center gap-2">
                    <div class="flex-1 bg-slate-100 h-1.5 rounded-full overflow-hidden"><div class="progress-bar bg-primary-500 h-full rounded-full" style="width:${pct}%"></div></div>
                    <span class="text-[10px] font-bold text-primary-600">${pct}%</span>
                    ${isRead ? `<button onclick="changeStatus('${book.id}','finished',event)" class="text-[10px] bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded font-bold active:scale-95">✅</button>` : ''}
                </div>
                ${isRead ? `<div class="flex gap-1.5 mt-2.5">
                    <button onclick="readSavedEpubFromCard('${book.id}',event)" class="flex-1 py-1.5 bg-primary-600 text-white rounded-lg text-[11px] font-bold active:scale-95 shadow-sm">📱 Читати</button>
                    <div class="relative flex-1">
                        <input type="file" id="epubFile_${book.id}" accept=".epub" class="hidden" onchange="handleFileSelectAndSave(event,'${book.id}')">
                        <button onclick="event.stopPropagation();document.getElementById('epubFile_${book.id}').click();" class="w-full py-1.5 bg-slate-100 rounded-lg text-[11px] font-bold active:scale-95">📥</button>
                    </div>
                </div>
                <div class="text-[10px] text-muted mt-1.5">⏱ ${formatTime(book.timeSpent)}</div>` : ''}
...
function render() {
    ...
    if (tabReading) tabReading.innerHTML = `📖 Читаю <span class="ml-1 text-[10px] opacity-70">${reading.length}</span>`;
    if (tabPlanned) tabPlanned.innerHTML = `🕒 В планах <span class="ml-1 text-[10px] opacity-70">${planned.length}</span>`;
    if (tabFinished) tabFinished.innerHTML = `✅ Прочитано <span class="ml-1 text-[10px] opacity-70">${finished.length}</span>`;
    ...
    if (currentLibraryTab === 'finished') {
        ...
        Object.keys(gr).sort((a, b) => { if (a === '—') return 1; if (b === '—') return -1; return b - a; }).forEach(y => {
            html += `<p class="font-black text-slate-200 text-sm mt-5 mb-2 tracking-widest">${y}</p><div class="${wc}">${gr[y].map(renderBookCard).join('')}</div>`;
        });
        c.innerHTML = html;
    } else {
...
const curatedCategories = {
    ...
    'Класика': ['"классика литературы"', 'Федор Достоевский', 'Лев Толстой', 'Михаил Булгаков', 'Антон Чехов', 'мировая классика']
};
...
async function loadRealRecommendations(cat = 'auto', btnId = 'rec_auto') {
    ...
    if (!list) return;
    list.innerHTML = '<div class="p-10 text-muted text-sm text-center animate-pulse">🔍 Шукаємо для вас...</div>';
...
async function fetchMoreRecommendations(isFirst = false) {
    ...
    if (finalBooks.length === 0 && isFirst) {
        ...
        finalBooks.forEach(i => {
            ...
            const div = document.createElement('div');
            div.className = "card p-3.5 flex gap-3 items-start cursor-pointer active:scale-[0.98] transition-transform fade-in";
            div.innerHTML = `
